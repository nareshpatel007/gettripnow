...
export function CountryDetail({ slug }: Props) {
    // Define state
    ...
    useEffect(() => {
        ...
        const fetchAttraction = async () => {
            try {
                ...
                // Fetch the data
                const response = await fetch("/api/country/single", {
                    method: "POST",
                    headers: {
    ...
    useEffect(() => {
        ...
        filterData();
        return () => controller.abort();
    }, [countryData, currentPage, sortFilter, resetFilter, appliedFilter]);
            ...
            {countryData?.name ? <>
                <div className="max-w-7xl mx-auto px-4 md:px-8 py-2 md:py-3">
                    <nav className="text-xs md:text-sm text-gray-500 overflow-x-auto whitespace-nowrap mb-3">
                        ...
                        <Link href="/country" className="hover:underline">Country</Link>
                        <span className="mx-1 md:mx-2">/</span>
                        <span className="text-gray-700">{countryData?.name}</span>
                    </nav>
                    <h1 className="text-2xl md:text-3xl font-bold text-[#1a2b49]">{countryData?.name}</h1>
                </div>
            </> : <>

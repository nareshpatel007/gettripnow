...
export function TourReviews({ summary, reviews, total_reviews, average_rating }: Review) {
    // Define state
    ...
    // convert to array + reverse (5 → 1)
    const ratingList = summary ? Object.entries(summary)
    ...
    return (
        <section className="py-8 border-b border-gray-200">
            ...
            <div className="grid md:grid-cols-2 gap-4 mb-8 border-b border-gray-200 hidden">
            ...
            <div className="grid md:grid-cols-3 gap-8 mb-8 pt-6">
                <div>
                    <div className="flex items-baseline gap-2 mb-2">
                        <span className="text-5xl font-bold text-gray-900">{Number(average_rating).toFixed(1)}</span>
                        <div className="flex">
                            {[1, 2, 3, 4, 5].map((star) => (
                                <Star
                                    key={star}
                                    className={`h-5 w-5 ${star <= Math.floor(average_rating) ? "fill-yellow-400 text-yellow-400" : "fill-gray-200 text-gray-200"}`}
                                />
                            ))}
                        </div>
                    </div>
                ...
                <div className="md:col-span-2 space-y-2">
                    {ratingList.map(({ rating, percentage }) => (
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6 hidden">
                    ...
                    <div key={index} className="py-6 first:pt-0">
                        ...
                        <div className="flex items-start justify-between mb-3">
                            <div className="flex items-start gap-3">
                                <div
                                    className={`w-10 h-10 rounded-full ${getAvatarColor(review.username)} flex items-center justify-center flex-shrink-0`}
                                >
                                    <span className="text-white font-semibold text-sm">{review.username.charAt(0).toUpperCase()}</span>
                                </div>
                                <div>
                                    <p className="text-sm font-medium text-gray-900">{review.username}</p>
                                    <p className="text-sm text-gray-500">{formatDate(review.published_date)}</p>
                                </div>
                            </div>
                        </div>
            ...
            <button
                ...
                className="mt-0 w-full py-2 border border-[#f53] rounded-lg font-medium text-[#f53] hover:bg-[#f53] hover:text-white transition-colors cursor-pointer"
            >
                Load more reviews
            </button>
                            ...
                            <div className="flex items-center gap-2 mt-4">
                                {lightbox.photos.map((_, index) => (
                                    <button
                                        ...
                                        className={`w-2.5 h-2.5 rounded-full transition-colors ${index === lightbox.currentIndex ? "bg-white" : "bg-white/40 hover:bg-white/60"
                                            }`}
                                    />
                                ))}
                            </div>
